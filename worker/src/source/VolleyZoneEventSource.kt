...
class VolleyZoneEventSource(
  ...
  private suspend fun parseLeagueTriangles(url: String): Map<String, Triangle> {
    ...
    val processedTriangles: List<Pair<Triangle?, List<VolleyZoneEvent>?>> =
      vzEvents.sortedBy(VolleyZoneEvent::id).groupBy(VolleyZoneEvent::triangleId).values.map {
        events ->
        if (events.size != 3) {
          val eventsStr =
    ...
    val triangles =
      if (config.attemptToFixBrokenTriangles) {
  ...
  private fun parseEvent(row: Element): VolleyZoneEvent? {
    ...
    val id =
      row.attr("data-comment").trim().let { comment ->
        ID_REGEX.find(comment)?.value
          ?: run {
            log.w(
  ...
  private fun parseResult(row: Element, scores: Element): VolleyZoneEvent? {
    ...
    if (
      event == null ||
        !row.hasClass("table-body") ||
        !scores.hasClass("list-centered_bottom_league")
    ) {
      return null
    }
    ...
    val (homeScores, awayScores) =
      if (results.size != totalSets.toInt()) {
        log.w(
    ...
    return events
      ...
      .mapNotNull { events ->
        if (events.size == 3) {
          ...
          log.e(
            "Could not fix fucked triangle, still got events != 3: ${events.map(VolleyZoneEvent::shortIdentity)}"
          )
          null
        }
      }
  ...
  private fun buildTriangle(url: String, events: Collection<VolleyZoneEvent>): Triangle {
  ...
  private fun resolveAddress(event: VolleyZoneEvent): String {
    val mapped =
      config.addresses.entries
        .firstOrNull { (k, _) -> event.venue.startsWith(k, ignoreCase = true) }
        ?.value
    return if (mapped != null) {
      ...
      log.w(
        "Unable to map address ${event.venue} for event ${event.id}. Falling back to raw venue $venue"
      )
      venue
    }
  }
  ...
  private fun buildResult(
    ...
    return if (event.homeSets == null || event.awaySets == null) {
      ...
      val teamAResult =
        TeamResult(id = homeTeamId, sets = event.homeSets, scores = event.homeScores)
      val teamBResult =
        TeamResult(id = awayTeamId, sets = event.awaySets, scores = event.awayScores)

      val (winner, loser) =
        if (teamAResult.sets > teamBResult.sets) {
  ...
  private data class VolleyZoneEvent(
    ...
    val shortIdentity
      get() = "$id $date $time $homeTeam - $awayTeam"
  }
  ...
  private data class Triangle(
    ...
    override val identity: String
      get() =
        "Triangle(id=$id, address=$address, host=$host, teams=$teams, events=${
          events.map(
  ...
  private data class TeamResult(val id: TeamId, val sets: UInt, val scores: List<UInt>?)
}
