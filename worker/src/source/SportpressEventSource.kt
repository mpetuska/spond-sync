...
class SportpressEventSource(
  ...
  override suspend fun provideEvents(
    ...
    teams.filter { it !in actualTeamNames }.forEach { log.w("Sportpress team $it not found!") }

    val teamIds = teamData.map(Team::id)
    val events =
      client
        .listEvents(after = start, before = end, seasons = listOf(season.id), limit = 100u)
        .filter { it.teams.any { id -> id in teamIds } }
        .toList()
    return teamData.associate { team ->
      log.d("Listing events for sportpress team ${team.identity}")
      val eventData =
        events
          .filter { event -> team.id in event.teams }
          .onEach { event -> log.v("Found event ${event.identity} for team ${team.identity}") }
          .map { event ->
            log.v("Building EventData for ${event.identity}")
            eventData(team, event).also { log.d("Built EventData for ${event.identity}") }
          }
  ...
  private suspend fun eventData(team: Team, event: Event): SourceEvent {
    ...
    val (triangleId, host) =
      event.day.split("\\s+".toRegex(), limit = 2).map { it.removePrefix("/") }.map(String::trim)
  ...
  private fun buildResult(event: Event, teamAId: TeamId, teamBId: TeamId): SourceEvent.Result? {
    return if (event.results.isEmpty()) {
      ...
      val teamASets =
        teamAScores.indices.map { if (teamAScores[it] > teamBScores[it]) 1u else 0u }.sum()
      val teamBSets =
        teamBScores.indices.map { if (teamAScores[it] < teamBScores[it]) 1u else 0u }.sum()
  ...
  private fun fetchTeams(season: Season, teams: Collection<String>): Flow<Team> {
    return client
      .listTeams(seasons = listOf(season.id), limit = 100u)
