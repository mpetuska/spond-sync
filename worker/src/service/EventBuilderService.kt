...
class EventBuilderService(
  ...
  private fun SourceEvent.start(): Instant =
    when {
  ...
  private fun SourceEvent.inviteTime(): Instant? {
    return (start - invitationDayBeforeStart.toInt().days).atNoon().takeIf {
      it > timeService.now()
    }
  }
  ...
  private fun SourceEvent.description() = buildString {
    ...
    if (result != null) {
      val (scoresA, scoresB, sets) =
        if (result.winnerId == teamAId) {
          Triple(
            result.winnerScores,
            result.loserScores,
            "${result.winnerSets}-${result.loserSets}",
          )
        } else {
          Triple(
            result.loserScores,
            result.winnerScores,
            "${result.loserSets}-${result.winnerSets}",
          )
        }
      if (scoresA != null && scoresB != null) {
        ...
        for (set in scoresA.indices) {
          appendLine(
            "  - ${scoresA[set].toString().padStart(2)}:${scoresB[set].toString().padStart(2)}"
          )
        }
      }
    }
