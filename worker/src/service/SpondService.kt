...
class SpondService(
  ...
  fun listMatches(group: Group, team: SubGroup, seasonStart: Instant): Flow<Event> =
    client
      .listEvents(
        groupId = group.id,
        subGroupId = team.id,
  ...
  suspend fun updateEvent(team: SubGroup, event: Event, sourceEvent: SourceEvent): Event? {
    log.v(
      "${sourceEvent.id}: Preparing merged spond event data for source event ${sourceEvent.identity}"
    )
    val updatedSpondEvent =
      runCatching { with(eventBuilderService) { sourceEvent.toEvent(event, team) } }
        .getOrElse {
          log.e(
            "${sourceEvent.id}: Failed to prepare merged spond event data for source event ${sourceEvent.identity}",
            it,
          )
          return null
        }
    log.d(
      "${sourceEvent.id}: Prepared merged spond event data for source event ${sourceEvent.identity}"
    )
    ...
    if (
      !eventBuilderService.modified(event, updatedSpondEvent) && !resultsModified && !forceUpdate
    ) {
      log.i(
    ...
    return try {
      val freshEvent =
        if (
          updatedSpondEvent.start > timeService.now() + 1.hours || resultsModified || forceUpdate
        ) {
      ...
      log.e(
        "${sourceEvent.id}: Failed to persist spond event update ${updatedSpondEvent.identity}",
        e,
      )
      null
    }
  ...
  suspend fun createEvent(
    group: Group,
    team: SubGroup,
    teamMembers: List<MemberId>,
    sourceEvent: SourceEvent,
  ): Event? {
    log.v("${sourceEvent.id}: Preparing spond event data for source event ${sourceEvent.identity}")
    val spondEvent =
      runCatching { with(eventBuilderService) { sourceEvent.toNewEvent(group, team, teamMembers) } }
        .getOrElse {
          log.e(
            "${sourceEvent.id}: Failed to prepare spond event data for source event ${sourceEvent.identity}",
            it,
          )
          null
        } ?: return null
    ...
    return try {
      ...
      log.e(
        "${sourceEvent.id}: Failed to persist new spond event creation ${spondEvent.identity}",
        e,
      )
      null
    }
