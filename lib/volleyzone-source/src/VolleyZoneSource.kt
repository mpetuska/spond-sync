...
import dev.petuska.spond.sync.core.model.Match.Result
import dev.petuska.spond.sync.core.model.Match.TeamResult
...
@ContributesBinding(ClubScope::class)
class VolleyZoneSource(
  ...
  private fun parseMatch(source: Url, row: Element): Match? {
    ...
    val friendly = comment.contains("friend", ignoreCase = true)
    val id =
      if (friendly) {
        "Friendly"
      } else {
    ...
    return Match(
        ...
        triangle = TriangleId(if (friendly) id else id.dropLast(1)),
        id = id,
        order = order.toUInt(),
